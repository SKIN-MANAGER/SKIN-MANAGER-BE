...
//
//@Repository
//public interface AnmFrmCmntRepo extends JpaRepository<AnmFrmCmntEntity, Long> {
//
//    @Query("select c  from AnmFrmEntity c left join fetch c.parentComment where c.commentId = :commentId and c.password = :password")
...
//
//    @Query("select c from AnmFrmEntity c left join fetch c.parentComment p where c.commentId = :commentId and p.commentId = :parentId and c.password = :password")
...
//
//    Page<AnmFrmEntity> findCommentEntityByForum(AnmFrmEntity forum, Pageable pageable);
//}
